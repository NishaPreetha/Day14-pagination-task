...
const viewer = (info) => {
    ...
    info.forEach(ele => {
        ...
        rows.setAttribute("class", "table table-bordered")
        for (let key in ele) {
            const cell = document.createElement("td");
            cell.innerText = ele[key];
...
parentBtn.setAttribute("class", "d-flex justify-content-center");
...
lastBtn.style.float = 'right';
lastBtn.addEventListener('click', () => {
    currPage = 10;
    loadInfo();
});
parentBtn.append(preBtn, firstBtn, nxtBtn, lastBtn)

const divElement = document.createElement('div');
divElement.append(preBtn, nxtBtn);
divElement.prepend(firstBtn);
divElement.appendChild(lastBtn);
...
const loadInfo = () => {
    ...
    info.slice(startIndex, endIndex).forEach((item) => {
        viewer([item]); // Pass item as an array to the reader function
    });
    document.body.append(divElement, parentBtn);
